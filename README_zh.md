...
DJA 在有害提示后附加一段短小的对抗后缀，并通过**双循环结构**对其进行迭代优化，整个攻击过程围绕"**动态**"这一核心思想展开：

- **外循环（动态目标）** — 基于*当前*对抗性提示，从目标模型的条件输出分布中采样候选回复，通过多目标综合评分器（有害性、语义相关性、具体性、连贯性、非拒绝性）动态选取高风险且与提示相关的目标。所选回复是模型自身在当前分布下已暴露的可行响应，而非外部预设的低概率模板。
- **内循环（梯度优化）** — 利用目标模型可微前向传播的梯度信号更新后缀，驱动模型更稳定地复现所选的动态目标。

每轮内循环结束后，DJA 重新采样并选择新目标，使优化目标**持续跟踪模型的动态输出分布**。在此基础上，DJA 进一步引入两种动态优化机制：当评分器未能找到满意目标时，**动态扩大采样预算**；当优化陷入停滞时，**动态扩展后缀长度**。三种动态机制协同作用，使攻击能够高效地发现并强化模型的有害行为模式，尤其在困难提示上表现更为突出。
